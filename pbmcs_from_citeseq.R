...
pbmc.rna <- readRDS(file = "data/bmcite_rna_counts.RDS")
pbmc.meta <- readRDS(file = "data/bmcite_metadata.RDS")
...
# Subset the celltypes of interest (the T cells)
Idents(pbmc) = "celltype.l2"
pbmc_naiveB <- subset(pbmc, idents = c("Naive B"))
pbmc_memoryB <- subset(pbmc, idents = c("Memory B"))
pbmc_cd4_memory <- subset(pbmc, idents = c("CD4 Memory"))
#pbmc_CD8T <- subset(pbmc, idents = c("CD8 T"))
#tcells <- subset(pbmc, idents = c("Naive B", "Memory B", "CD8 T"))
# TODO consider the naiveB mixture by itself and also in a larger context
# 1. mix in the B cells also
# 2. in the context of all celltypes

# TODO # would be cool to check random matrix theory things on them,
# and mean/variance things as well
...
# 2 # Majority naiveB, minority cd4_memory
naiveB_1900_cd4_memory_49 <- merge_majority_minority(x=pbmc_naiveB, y=cd4_memory_49, cell_ids = c("naiveB", "cd4_memory"))
...
i = 18
j = 1
# when looking at the genes unique to a harder dataset compared to an easy oone, and plotting them in the easier dataset,
...
# what is the effect of number of critical genes on the latent separability
for (i in 1:length(cluster_s_list)) {
  violin_plot_data_pcs = data.frame()
  cluster_s = cluster_s_list[[i]]
  cluster_ident = names(cluster_s_list)[i]
  print(cluster_ident)
  ...
  # track the mixing in the latent space, compute for the perturbed.minority cells only
  num_minority_cells = sum(cluster_s@meta.data$celltype.l2==minority_celltype)
  n_critical_genes = seq(from=100, to=2000, by=100)
  violin_plot_data = data.frame()
  for(n in n_critical_genes) {
    print(n)
    critical_genes_n = rownames(cluster_s@assays$RNA@data)[
      head(order(abs(cor_vec), decreasing = T), n)] 
    VariableFeatures(cluster_s) = critical_genes_n
    npcs = 2
    cluster_s <- RunPCA(cluster_s, npcs=npcs)
    cluster_s <- FindNeighbors(cluster_s, compute.SNN = TRUE, dims = 1:npcs)
    tmp = apply(cluster_s@graphs[[1]][cluster_s@meta.data$celltype.l2==minority_celltype,], 1, 
                function(x) {cluster_s@meta.data$celltype.l2[x == 1]})
    # vector of length number of minoruty cells containing the proportion of their neighborst might minority label
    prop_minority_neighbors = apply(tmp, 2, function(x) {mean(x==minority_celltype)})
    numPCs = rep(n, num_minority_cells)
    violin_plot_data = rbind(violin_plot_data, cbind(prop_minority_neighbors, n))
    print(paste0('done with ', n, ' critical genes!'))
  }
  # Plots
  # 1. across num critical genes for the same num_minority cells
  violin_plot_data$n = as.factor(violin_plot_data$n)
  p <- ggplot(violin_plot_data, aes(x=n, y=prop_minority_neighbors)) + geom_boxplot(width=0.1)#+ geom_violin()
  p = p + stat_summary(fun=median, geom="point", size=2, color='red') 
  p = p + ggtitle(paste0(num_minority_cells, " ", minority_celltype, " cells"))
  p = p + xlab('Num Critical Genes') + ylab(paste0("Fraction of ", minority_celltype, " neighbors"))
  p
  ggsave(paste0(cluster_dir_path, "prop_neighbors_acrosscritgenes.png"))
}



# Next
# Save data for python analysis - done
# do the above analysis with the top most DE genes from comparing the two full clusters
# does including more or less hvgs help?
# plot with barplots and red dots for variable number of variable genes
# are the critical genes ranks consistent across datasets?
# alternatively, what is the overlap in critical genes across datasets?
# are the critical genes differentially expessed in the mix of two whole?
# cor_vec across datasets, what is the overlap


#mean_counts_genes_unique_to_j_cl_i_celltype_df 
plot_mean_lognormcounts_genes_unique_to_x_cl_y <- function(x, y, label='celltype',
  variable_genes_list, cluster_s_list, col_batch, col_perturb){
  genes_int_xy = intersect(variable_genes_list[[x]], variable_genes_list[[y]])
  genes_unique_to_x = setdiff(variable_genes_list[[x]], genes_int_xy)
  cluster_s_y = cluster_s_list[[y]]
  cells_majority = cluster_s_y$celltype.l2!=minority_celltype
  cells_minority = cluster_s_y$celltype.l2==minority_celltype
  
  if (label=='batch') {
    batch_df = data.frame(rbind(
      cbind(rowMeans(as.matrix(cluster_s_y@assays$RNA@data[genes_unique_to_x, cluster_s_y$batch==0])), 
            rep(0, length(genes_unique_to_x))),
      cbind(rowMeans(as.matrix(cluster_s_i@assays$RNA@data[genes_unique_to_x, cluster_s_y$batch==1])), 
            rep(1, length(genes_unique_to_x)))))
    colnames(batch_df) = c('mean_lognormcounts', 'batch')
    batch_df[,'batch'] = as.factor(batch_df[,'batch'])
    p <- ggplot(batch_df, aes(x=batch, y=mean_lognormcounts, 
                              fill=batch)) + geom_violin() + scale_fill_manual(values=col_batch)
    return(p)
  }
  if (label=='celltype') {
    celltype_df = data.frame(rbind(
      cbind(rowMeans(as.matrix(cluster_s_y@assays$RNA@data[genes_unique_to_x, cells_majority])), 
            rep(0, length(genes_unique_to_x))),
      cbind(rowMeans(as.matrix(cluster_s_i@assays$RNA@data[genes_unique_to_x, cells_minority])), 
            rep(1, length(genes_unique_to_x)))))
    colnames(celltype_df) = c('mean_lognormcounts', 'celltype')
    celltype_df[,'celltype'] = as.factor(celltype_df[,'celltype'])
    p <- ggplot(celltype_df, aes(x=celltype, y=mean_lognormcounts, 
                                 fill=celltype)) + geom_violin() + scale_fill_manual(values=col_perturb)
    return(p)
  } else {
    print("Choose either 'batch' or 'celltype' for label.")
  }
}
plot_mean_lognormcounts_genes_unique_to_x_cl_y(x=18, y=1, label='batch', 
                                               variable_genes_list, cluster_s_list, col_batch, col_perturb)
plot_mean_lognormcounts_genes_unique_to_x_cl_y(x=18, y=1, label='celltype', 
                                               variable_genes_list, cluster_s_list, col_batch, col_perturb)
...
# Violin plots
Idents(cluster_s)
VlnPlot(cluster_s, features = c("TCL1A", "IGHD", "B2M", "CLECL1"), cols = c("red", "gray"))
Idents(cluster_s) = 'batch'
VlnPlot(cluster_s, features = c("TCL1A", "IGHD", "B2M", "CLECL1"))#, cols = col_batch)
#VlnPlot(pbmc, idents = c("Naive B", "Memory B"), features = c("JUNB", "USP10", "NELL2"))

#==== Can we find the DE genes? ====
# DE based on batch labels
Idents(cluster_s) = 'batch'
markers_batch_label = FindMarkers(cluster_s, ident.1 = '0', only.pos = FALSE, test.use = 't')
head(markers_batch_label)

markers_batch_label = FindMarkers(cluster_s, ident.1 = '0', only.pos = FALSE, 
                                  logfc.threshold = log(1.01), test.use = 't')
head(markers_batch_label)
VlnPlot(cluster_s, features = c("MRPL51"))#, cols = col_batch)
Idents(cluster_s) = 'celltype.l2'
VlnPlot(cluster_s, features = c("MRPL51"), col_batch)
FeaturePlot(cluster_s, features = c("MRPL51"))
FeaturePlot(cluster_s, features = c("RP5-887A10.1"))
#markers_batch_label[as.character(gene_names),]
#gene_names[which(markers_batch_label[as.character(gene_names),]$p_val_adj<0.05)]
#FeaturePlot(cluster_s, features = 'Ar')
#FeaturePlot(cluster_s, features = 'Arsb')
#FeaturePlot(cluster_s, features = 'Sept7')
#num_de = sum(markers_batch_label[as.character(gene_names), 'p_val_adj']<0.05)
#pie_df <- data.frame(group = c("DE", "Not DE"), value = c(num_de, length(gene_names)-num_de))
#ggplot(pie_df, aes(x="", y=value, fill=group))+geom_bar(width=1, stat="identity")+coord_polar("y",start=0)+scale_fill_manual(values=col_de)
#par(mfrow=c(1, 1))
#plot(x=markers_batch_label[as.character(gene_names), 'p_val_adj'], y=df[df$VariableFeatures.cluster_s.%in%as.character(gene_names), "gene_total"])
##hist(markers_batch_label[gene_names,'p_val_adj'], bins=100)
##boxplot(markers_batch_label$p_val_adj, ylab('p_val_adj'))
##ggplot(markers_batch_label, aes(x=p_val_adj, y=p_val_adj)) + geom_violin()

# DE based on perturbed label
Idents(cluster_s) = 'celltype.l2'
markers_perturb_label = FindMarkers(cluster_s, ident.1 = minority_celltype, 
                                    only.pos = FALSE, test.use = 't')
head(markers_perturb_label)
gene_name = "ANXA1"
print(VlnPlot(cluster_s, features = gene_name, group.by = 'batch', 
              cols = col_batch) + ggtitle(paste0(gene_name, ' Original labels after perturb')))
print(VlnPlot(cluster_s, features = gene_name, group.by = 'celltype.l2',
              cols = c("red", "gray")) + ggtitle(paste0(gene_name, ' True perturbed labels')))

#markers_perturb_label[as.character(gene_names),]
#num_de = sum(markers_perturb_label[as.character(gene_names), 'p_val_adj']<0.05)
#pie_df <- data.frame(group = c("DE", "Not DE"), value = c(num_de, length(gene_names)-num_de))
##ggplot(pie_df, aes(x="", y=value, fill=group))+geom_bar(width=1, stat="identity")+coord_polar("y",start=0)+scale_fill_manual(values=col_de)
##markers_perturb_label = FindMarkers(cluster_s, ident.1 = '0', only.pos = FALSE)
